...
df = pd.DataFrame(columns=['Method', 'Function', 'Learning Rate', 'Initial Point', 'Optimal Point', 'Function Value', 'Iterations', 'Execution Time'])

df.loc[0] = ['Gradient Descent', 'x^2 * y^2 * log(x^2 + y^2)', '0.1', f'({x0}, {y0})', f'({x_opt1[-1]}, {y_opt1[-1]})', f1(x_opt1[-1], y_opt1[-1]), num_iterations1, execution_time1]
df.loc[1] = ['Gradient Descent', 'x^2 * y^2 * log(8x^2 + 3y^2)', '0.1', f'({x0}, {y0})', f'({x_opt2[-1]}, {y_opt2[-1]})', f2(x_opt2[-1], y_opt2[-1]), num_iterations2, execution_time2]
...
df.loc[2] = ['Gradient Descent', 'x^2 * y^2 * log(x^2 + y^2)', 'Golden section search', f'({x0}, {y0})', f'({x_opt1[-1]}, {y_opt1[-1]})', f1(x_opt1[-1], y_opt1[-1]), num_iterations1, execution_time1]
df.loc[3] = ['Gradient Descent', 'x^2 * y^2 * log(8x^2 + 3y^2)', 'Golden section search', f'({x0}, {y0})', f'({x_opt2[-1]}, {y_opt2[-1]})', f2(x_opt2[-1], y_opt2[-1]), num_iterations2, execution_time2]
...
df.loc[4] = ['Scipy Minimize', 'x^2 * y^2 * log(x^2 + y^2)', 'Nelder-Mead', f'({x0}, {y0})', f'({res1.x[0]}, {res1.x[1]})', f1(res1.x[0], res1.x[1]), res1.nit, res1.nfev]
df.loc[5] = ['Scipy Minimize', 'x^2 * y^2 * log(8x^2 + 3y^2)', 'Nelder-Mead', f'({x0}, {y0})', f'({res2.x[0]}, {res2.x[1]})', f2(res2.x[0], res2.x[1]), res2.nit, res2.nfev]
...
df.loc[6] = ['Coordinate Descent', 'x^2 * y^2 * log(x^2 + y^2)', 'Golden section search', f'({x0}, {y0})', f'({x_opt1}, {y_opt1})', f1(x_opt1, y_opt1), num_iterations1, execution_time1]
df.loc[7] = ['Coordinate Descent', 'x^2 * y^2 * log(8x^2 + 3y^2)', 'Golden section search', f'({x0}, {y0})', f'({x_opt2}, {y_opt2})', f2(x_opt2, y_opt2), num_iterations2, execution_time2]
...
df.loc[8] = ['Gradient Descent', 'x^3 * y^3 * z^3', '0.1', f'({init_points[0]}, {init_points[1]}, {init_points[2]})', f'({points[-1][0]}, {points[-1][1]}, {points[-1][2]})', f3(points[-1][0], points[-1][1], points[-1][2]), num_iterations, execution_time]
...
df.loc[9] = ['Gradient Descent', 'x**3 + y**3 + np.sqrt(x**4 + y**4)', 'Golden section search', f'({x0}, {y0})', f'({x_bad[-1]}, {y_bad[-1]})', f_bad(x_bad[-1], y_bad[-1]), num_iterations_bad, execution_time_bad]
...
df.loc[10] = ['Gradient Descent', 'x^2 * y^2 * log(x^2 + y^2) with noise', 'Golden section search', f'({x0}, {y0})', f'({x[-1]}, {y[-1]})', f1_with_noise(x[-1], y[-1]), num_iterations, execution_time]
